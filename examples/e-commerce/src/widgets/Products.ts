...
export const products = hits({
  ...
  templates: {
    item: `
<a href="{{clippingsUrl}}" class="hit-link" target="_blank">
  <article class="hit">
    <header class="hit-image-container">
      <img
        src="{{pictureUrl263x263}}"
        alt="{{productName}} {{categoryLevel1}} by {{designerName}} - image"
        class="hit-image"
        title="{{productName}} - {{categoryLevel2}} - {{brandName}}">
    </header>

    <div class="hit-info-container">
      <h1>{{#helpers.highlight}}{ "attribute": "productName" }{{/helpers.highlight}}</h1>
      <h1>{{productName}}</h1>
      <h2>by {{designerName}} for {{brandName}}</h2>
      <h2>{{categoryLevel1}} > {{categoryLevel2}}</h2>
